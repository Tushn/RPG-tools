...
import org.xml.sax.SAXException;
    ...
    private String xmlPathname2;
    private String xmlPathname3;
    private String xmlPathname4;
    ...
    public LerXML( String path, String path2, String path3, String path4 ) {  
        xmlPathname = path;
        xmlPathname2 = path2;
        xmlPathname3 = path3;
        xmlPathname4 = path4;
     ...
     */
    public void iniciarXML() throws ParserConfigurationException, SAXException, IOException{
    ...
    public Vector lerXMLEventosChave(int subtipo)throws ParserConfigurationException, SAXException, IOException, Exception{
        iniciarXML();
        
        Vector chaves = new Vector();
        
        NodeList nodeChave = elem.getElementsByTagName( "chave" );
        Element tagPlayer = (Element) nodeChave.item( subtipo );
        int id = Integer.parseInt( tagPlayer.getAttribute( "id" ) );
        Integer idteste = new Integer( getChildTagValue( tagPlayer, "idteste" ) );
        Integer trueid = new Integer( getChildTagValue( tagPlayer, "trueid" ) );
        Integer falseid = new Integer( getChildTagValue( tagPlayer, "falseid" ) );
        String descricaotrueid = getChildTagValue( tagPlayer, "descricaotrueid" );
        String descricaofalseid = getChildTagValue( tagPlayer, "descricaofalseid" );
        
        EventoChave chave = new EventoChave(idteste, trueid, falseid, descricaotrueid, descricaofalseid);
        chaves.addElement( chave );
        
        return chaves;
    }
    
    public Vector lerXMLTestarValores(int subtipo) throws ParserConfigurationException, SAXException, IOException, Exception{
        iniciarXML();
        
        Vector testes = new Vector();
        
        NodeList nodeteste = elem.getElementsByTagName( "teste" );
        Element tagPlayer = (Element) nodeteste.item( subtipo );
        int id = Integer.parseInt( tagPlayer.getAttribute( "id" ) );
        Integer trueid = new Integer( getChildTagValue( tagPlayer, "trueid" ) );
        Integer falseid = new Integer( getChildTagValue( tagPlayer, "falseid" ) );
        Integer caracteristica = new Integer(  getChildTagValue( tagPlayer, "caracteristica" ) );
        Integer dificuldade = new Integer (  getChildTagValue( tagPlayer, "dificuldade" ) );
        
        TestarValores teste = new TestarValores(id, caracteristica, dificuldade, trueid, falseid);
        testes.addElement( teste );
        
        return testes;
    }
    
    public Vector lerXMLAlteracao(int tipo, int subtipo) throws Exception{
        iniciarXML();
        
        Vector alteracoes = new Vector();
        
        NodeList nodealteracao;
        if(tipo==4) // Modificacao de caracteristicas
            nodealteracao = elem.getElementsByTagName( "modificacao" );
        else
            nodealteracao = elem.getElementsByTagName( "incrementodecremento" );
        
        Element tagPlayer = (Element) nodealteracao.item( subtipo );
        int id = Integer.parseInt( tagPlayer.getAttribute( "id" ) );
        Integer atributo = new Integer( getChildTagValue( tagPlayer, "atributo") );
        Integer valor = new Integer( getChildTagValue( tagPlayer, "valor") );
        Integer idoutro = new Integer( getChildTagValue( tagPlayer, "idoutro") );
        
        Alteracao alteracao;
        if(tipo==4)
            alteracao = new Modificacao(id,atributo,valor,idoutro);
        else
            alteracao = new IncrementoDecremento(id,atributo,valor,idoutro);
        System.out.print("id: "+id+", atributo: "+atributo+", valor: "+valor+", idoutro: "+idoutro+"\n");
        alteracoes.addElement( alteracao );
        return alteracoes;
    }
    
    public Vector lerFichaXMLPersonagem(int i) throws Exception { 
        iniciarXML();
        ...
        NodeList nodePoder = elem.getElementsByTagName( "poder" );
        Poder[] poderes;
            // Ajustes dos dados do personagem oriundos do XML
            Element tagPlayer = (Element) nl.item( i );
            int idTemp = Integer.parseInt( tagPlayer.getAttribute( "id" ) );  
            ...
            Integer maxPoder = new Integer( getChildTagValue( tagPlayer, "maxpoder" ) );
            Integer minPoder = new Integer( getChildTagValue( tagPlayer, "minpoder" ) );
            ...
            PersonagemJogador player = new PersonagemJogador(nome, descricao, habilidade, energia, sorte, magia, i, raca, kit, maxPoder, minPoder);  
            player = lerPoder(player, minPoder, maxPoder);
            fichas.addElement( player );   

        return fichas;  
    } 
    
    public PersonagemJogador lerPoder(PersonagemJogador player, int minPoder, int maxPoder) throws SAXException, IOException, ParserConfigurationException, Exception{
                player = player;
                if(player.getMagia()>0){
                dbf = DocumentBuilderFactory.newInstance();  
                db = dbf.newDocumentBuilder();
                System.out.println("xmlPathname4: "+xmlPathname4);
                doc = db.parse( xmlPathname4 );
                elem = doc.getDocumentElement();
                
                NodeList nodePoder = elem.getElementsByTagName( "poder" );

                Element tagPlayer = (Element) nodePoder.item( minPoder );
                int idTempPoder = Integer.parseInt( tagPlayer.getAttribute( "id" ) );
                String nomePoder = getChildTagValue( tagPlayer, "nome" );
                String descricaoPoder = getChildTagValue( tagPlayer, "descricao" );
                Integer tipoId = new Integer( getChildTagValue( tagPlayer, "idtipo" ) );
                String tipo = getChildTagValue( tagPlayer, "tipo" );
                Integer custo = new Integer( getChildTagValue( tagPlayer, "custo" ) );
                Integer valor = new Integer( getChildTagValue( tagPlayer, "valor" ) );
                Integer contRandom = new Integer( getChildTagValue(tagPlayer, "contrandom"));
                
                Poder poder = new Poder(idTempPoder, nomePoder, descricaoPoder, tipoId, tipo, custo, valor, contRandom);
                player.setPoder(poder);
                for(int i = minPoder+1; i<= maxPoder; i++){
                    tagPlayer = (Element) nodePoder.item( i );
                    idTempPoder = Integer.parseInt( tagPlayer.getAttribute( "id" ) );
                    nomePoder = getChildTagValue( tagPlayer, "nome" );
                    descricaoPoder = getChildTagValue( tagPlayer, "descricao" );
                    tipoId = new Integer( getChildTagValue( tagPlayer, "idtipo" ) );
                    tipo = getChildTagValue( tagPlayer, "tipo" );
                    custo = new Integer( getChildTagValue( tagPlayer, "custo" ) );
                    valor = new Integer( getChildTagValue( tagPlayer, "valor" ) );
                    contRandom = new Integer( getChildTagValue(tagPlayer, "contrandom"));

                    poder =  new Poder(idTempPoder, nomePoder, descricaoPoder, tipoId, tipo, custo, valor, contRandom);
                    player.setPoder(poder);
           }
        }
        return player;
    ...
    public Raca lerXMLRaca(int id) throws ParserConfigurationException, SAXException, IOException, Exception{
        iniciarXML();
        
        NodeList nodeRaca = elem.getElementsByTagName( "raca" );
        Element tagPlayer = (Element) nodeRaca.item( id );
        int idTemp = Integer.parseInt( tagPlayer.getAttribute( "id" ) );
        String nome = getChildTagValue( tagPlayer, "nome" );
        Integer habilidade = new Integer( getChildTagValue( tagPlayer, "habilidade" ) );
        Integer energia = new Integer( getChildTagValue( tagPlayer, "energia" ) );
        ...
        Raca raca = new Raca(nome, descricao, habilidade, energia, sorte, magia);
        
        return raca;
    }
    
    public Kit lerXMLKit(int id) throws ParserConfigurationException, SAXException, IOException, Exception{
        iniciarXML();
        
        NodeList nodeKit = elem.getElementsByTagName( "kit" );
        Element tagPlayer = (Element) nodeKit.item( id );
        int idTemp = Integer.parseInt( tagPlayer.getAttribute( "id" ) );
        String nome = getChildTagValue( tagPlayer, "nome" );
        Integer habilidade = new Integer( getChildTagValue( tagPlayer, "habilidade" ) );
        Integer energia = new Integer( getChildTagValue( tagPlayer, "energia" ) );
        ...
        Kit kit = new Kit(nome, descricao, habilidade, energia, sorte, magia);
